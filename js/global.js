...
function calculateIt() {
		...
		if (job === 0) {
			...
			// minmax
			maxdmg = Math.floor(parseInt(maxresult * skillattack * elementamp *
				elemwandbonus));
			mindmg = Math.floor(parseInt(minresult * skillattack * elementamp *
				elemwandbonus));
			...
			mobamount = mobamount[monsteramount]
				// min-max-avg dmg 
			var mindmg = ((totalint * 1.2 + totalluk * 4) * totalmagic / 1000 *
				heallevel * mobamount);
			var maxdmg = ((totalint * 4.8 + totalluk * 4) * totalmagic / 1000 *
				heallevel * mobamount);
			checkWeakness(damagetype, m);
			// def reduxx + avg
			maxdmg = (maxdmg - monsterlist[m].mdef * 0.5 * (1 + 0.01 * d))
